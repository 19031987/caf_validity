    ...
    public Scancaf getCaf(@RequestBody Scancaf id) {
        ...
        try {
            ...
            if (checkIfBarcodeExits(id.getCafbarcode())) {
                ...
                if (getdata != null) {
                    scan.setActivationdate((String) getdata[3]);
                    scan.setCustomername((String) getdata[1]);
                    scan.setCaftype((String) getdata[4]);
                    scan.setCafbarcode((String) getdata[2]);
                    BigInteger mobno = (BigInteger) getdata[0];
                    scan.setMobilenumber(mobno.longValue());

                    if (scan.getCaftype().trim().equalsIgnoreCase("P")) {
                        scan.setCaftype("Active");
                    } else if (scan.getCaftype().trim().equalsIgnoreCase("R")) {
                        scan.setCaftype("RV");
                    } else if (scan.getCaftype().trim().equalsIgnoreCase("<1 Year")) {
                        scan.setCaftype("L1");
                    } else if (scan.getCaftype().trim().equalsIgnoreCase(">3")) {
                        scan.setCaftype("G3y");
                    } else if (scan.getCaftype().trim().equalsIgnoreCase("1 to 2")) {
                        scan.setCaftype("1-2y");
                    } else if (scan.getCaftype().trim().equalsIgnoreCase("2 to 3")) {
                        scan.setCaftype("2-3y");
                    }
            ...
            log.info("non unique value" + nure);
        }
